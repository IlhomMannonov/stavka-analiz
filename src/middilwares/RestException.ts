...
export class RestException extends Error {
    ...
    static notFound(resourceKey: string): RestException {
        return new RestException(`Resource ${resourceKey} not found`, HttpStatus.NOT_FOUND);
    }
    ...
    static badRequest(resourceKey: string): RestException {
        return new RestException(`Bad request for ${resourceKey}`, HttpStatus.BAD_REQUEST);
    }
